PROGRAM MAIN
!.. Defining vars for use, names unchanged

      integer ionmax

      double precision temp,den,xmass(1:5),zion(1:5),aion(1:5)
      double precision pep,xne,eta,opac,ocond,orad,conductivity

      character(100)  tempstring, rhostring, pepstring, etastring,xnestring

      call get_command_argument(1,tempstring)   !first, read in the two values
      call get_command_argument(2,rhostring)   !first, read in the two values
      call get_command_argument(3,pepstring)   !first, read in the two values
      call get_command_argument(4,etastring)   !first, read in the two values
      call get_command_argument(5,xnestring)   !first, read in the two values
...
!      write(*,*) 'Beginning opacity program'

      call sig99(temp,den,xmass,zion,aion,ionmax,pep,xne,eta,orad, &
          ocond, opac, conductivity)      
...
subroutine sig99(temp,den,xmass,zion,aion,ionmax,pep,xne,eta, &
      ...
      PRINT *, opac, orad, ocond, conductivity
